...
public class Redirector : MonoBehaviour {
    ...
    private float TowardWallCost(Vector3 playerPos,float width,float length,float distanceDampFactor)
    {
        //Debug.Log(playerPos);
        float result;
        float xAxisAbs = Mathf.Abs(playerPos.x);
        float zAxisAbs = Mathf.Abs(playerPos.z);
        if (xAxisAbs > width || zAxisAbs > length) result = distanceCostMax;
        else
        {
            float widthDistance = Mathf.Abs(width - xAxisAbs);
            float lengthDistance = Mathf.Abs(length - zAxisAbs);
            result = widthDistance <= lengthDistance ? (1.0f / widthDistance * distanceDampFactor) : (1.0f / lengthDistance * distanceDampFactor);
